...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add') {
        $sql = "INSERT INTO products (ProductName, Description, Price, Category, StockQuantity, ImageURL) VALUES ('$productName', '$description', $price, '$category', $stockQuantity, '$imageURL')";
        if ($conn->query($sql)) {
            echo "<div class='success-message'>Product added successfully!</div>";
        } else {
            echo "<div class='error-message'>Add error: " . $conn->error . "</div>";
        }
    } elseif ($_POST['action'] === 'update') {
        $id = intval($_POST['id']);
        $sql = "UPDATE products SET ProductName='$productName', Description='$description', Price=$price, Category='$category', StockQuantity=$stockQuantity, ImageURL='$imageURL' WHERE id=$id";
        if ($conn->query($sql)) {
...
<head>
    ...
    <title>KFC Menu Management</title>
    ...
    <div class="main-container">
        ...
        <div class="form-container">
            ...
            <form action="product.php" method="post">
                ...
                <div class="form-group"><label>Price (VND):</label><input type="number" step="0.01" name="price" required></div>
                <div class="form-group"><label>Category:</label><input type="text" name="category" required></div>
                <div class="form-group"><label>Stock Quantity:</label><input type="number" name="stockQuantity" required></div>
                <div class="form-group"><label>Image URL:</label><input type="text" name="imageURL"></div>
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['id'])) {
            ...
            if ($result && $result->num_rows > 0) {
                ...
                echo '<div class="form-group"><label>Price (VND):</label><input type="number" step="0.01" name="price" value="' . htmlspecialchars($product['Price']) . '" required></div>';
                ...
                echo '<div class="form-group"><label>Image URL:</label><input type="text" name="imageURL" value="' . htmlspecialchars($product['ImageURL']) . '">';
                echo '</div>';
            ...
            <div class="table-container">
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>\n<td>" . htmlspecialchars($row['ProductName']) . "</td>\n<td>" . htmlspecialchars(substr($row['Description'], 0, 50)) . "...</td>\n<td>$" . number_format($row['Price'], 2) . "</td>\n<td>" . htmlspecialchars($row['Category']) . "</td>\n<td>" . htmlspecialchars($row['StockQuantity']) . "</td>\n<td class='action-buttons'><a href='product.php?action=edit&id=" . $row['id'] . "' class='edit-btn'>Edit</a> <a href='product.php?action=delete&id=" . $row['id'] . "' class='delete-btn' onclick='return confirm(\"Are you sure you want to delete this product?\")'>Delete</a></td>\n</tr>\n";
                    }
                    echo "</tbody>\n</table>";
                } else {
